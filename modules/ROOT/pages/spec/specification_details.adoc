...
The workflow definition also explicitly defines when a workflow instance should be completed. By default, instances should be completed once there are no active workflow paths (all active paths reach a state containing the default xref:spec/structure/state_definitions/end_definition.adoc[end definition]), or if the defined xref:spec/workflow_timeouts.adoc[`workflowExecTimeout`] time is reached. Other ways, such as using the `terminate` property of the xref:spec/structure/state_definitions/end_definition.adoc[end definition] to terminate instance execution, or defining an xref:spec/workflow_timeouts.adoc[`workflowExecTimeout`] property are also possible.
